...
- Customize API generation tests based on platform capabilities

## Troubleshooting

### Tests Timeout
- Increase the workflow `timeout-minutes` value (default example shows 75 minutes)
- Reduce `TEST_JOBS` to decrease resource contention
- Check if specific tests are hanging and exclude them

### Multi-Worker Tests Fail
- Verify the system supports multi-worker VPP configuration
- Check if `VPP_WORKER_COUNT` is properly supported in the VPP build
- Ensure sufficient system resources for multi-worker testing
- Try reducing `VPP_WORKER_COUNT` to 1 for debugging
- Verify CPU cores are available for the specified worker count

### API Generation Tests Fail
- Verify Python environment is properly configured
- Check if `src/tools/vppapigen/test_vppapigen.py` exists and is executable
- Ensure API generation dependencies are installed

### OS Mismatch Issues
- Verify `OS_ID` and `OS_VERSION_ID` are set correctly by setup actions
- Check OS pattern strings match the expected format (`os-version`)
- Enable debug logging to see which tests are being skipped

## Key Differences from vpp-test Action

This `vpp-maketest` action differs from the standard `vpp-test` action in several ways:

### Enhanced Configuration
- **Configurable Retries**: `TEST_RETRIES` parameter allows customizing retry behavior
- **Worker Count Control**: `VPP_WORKER_COUNT` parameter for precise multi-worker configuration
- **Simplified Multi-Worker Logic**: Uses only `VPP_WORKER_COUNT` method (no auto-detection)

### Implementation Changes
- **Fixed Retry Logic**: Standard tests use hardcoded 3 retries, multi-worker tests use configurable `TEST_RETRIES`
- **Consistent Parameter Usage**: All retry references use the `TEST_RETRIES` parameter for multi-worker tests
- **Streamlined Multi-Worker**: Removes auto-detection between `VPP_WORKER_CONFIG` and `VPP_WORKER_COUNT`

## Notes

- Tests automatically retry based on `TEST_RETRIES` configuration (default: 3 times)
- Log compression is enabled by default to conserve storage space
- The action uses `VPP_WORKER_COUNT` method for multi-worker configuration
- Test execution is optimized for CI/CD environments with unattended operation
- Multi-worker tests support configurable worker counts via `VPP_WORKER_COUNT`
